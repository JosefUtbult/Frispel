...
def googleresult_to_dateTime(googleresult):
	...
	try:
		...
		return ( RFC3339_parse(googleresult['start']['dateTime']).replace(tzinfo=None), RFC3339_parse(googleresult['end']['dateTime']).replace(tzinfo=None) )
	except KeyError as e:
		return ( datetime.strptime(googleresult['start']['date'], "%Y-%m-%d"), datetime.strptime(googleresult['end']['date'], "%Y-%m-%d").replace(hour=23, minute=58) - timedelta(days=1))
